...
const ProjectCard = memo(({ project, index }) => {
  const safeTitle = project?.title ? project.title.toUpperCase() : "UNTITLED";
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 group-hover:scale-105 transition-transform duration-500 ease-out flex items-center justify-center">
            {project?.image ? (
                <img src={project.image} alt={project?.title || "Project Image"} className="w-full h-full object-cover" loading="lazy" />
            ) : (
                <span className="text-slate-400 dark:text-slate-600 font-mono text-sm tracking-widest">{safeTitle}</span>
            )}
        </div>
      </div>
        ...
        <div className="flex items-center gap-6 mt-4">
            {project?.demoLink && (
              <a
                ...
                className="flex items-center gap-2 text-primary-blue text-sm font-bold tracking-wide group/link hover:text-blue-700 dark:hover:text-blue-400 transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-slate-900 focus-visible:ring-primary-blue rounded-sm focus:outline-none"
              >
                View Project
            ...
            {project?.githubLink && (
               ...
               target="_blank"
               rel="noopener noreferrer"
               className="text-slate-500 dark:text-slate-400 text-sm font-semibold tracking-wide hover:text-slate-900 dark:hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-slate-900 focus-visible:ring-primary-blue rounded-sm focus:outline-none"
             >
               Source Code
             </a>
            )}
