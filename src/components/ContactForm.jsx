...
export default function ContactForm({ onSubmit }) {
  const contactServiceId =
    import.meta.env.VITE_CONTACT_SERVICE_ID || "xeelgjya";
  ...
  const handleLocalSubmit = async (e) => {
    ...
    if (!contactServiceId) {
      console.warn(
        "Contact form submission failed: VITE_CONTACT_SERVICE_ID is not defined.",
      );
      alert(
        "The contact form is currently not configured. Please use a different method to contact me.",
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `https://formspree.io/f/${contactServiceId}`,
        {
          method: "POST",
          body: formData,
          headers: {
      ...
      <form onSubmit={handleLocalSubmit} className="space-y-6" noValidate>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative rounded-md shadow-sm">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <HiUser
                  className={`text-lg ${errors.name ? "text-rose-500" : "text-gray-400 dark:text-slate-500"}`}
                />
              </div>
              <input
            ...
            {errors.name && (
              <p
                id="name-error"
                className="text-xs text-rose-500 mt-1 ml-1"
                role="alert"
              >
                {errors.name}
              </p>
          ...
          <div className="space-y-2">
            ...
            <div className="relative rounded-md shadow-sm">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <HiMail
                  className={`text-lg ${errors.email ? "text-rose-500" : "text-gray-400 dark:text-slate-500"}`}
                />
              </div>
              <input
        ...
        <div className="space-y-2">
          ...
          <div className="relative rounded-md shadow-sm">
            <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
              <HiFolder
                className={`text-lg ${errors._subject ? "text-rose-500" : "text-gray-400 dark:text-slate-500"}`}
              />
            </div>
            <input
