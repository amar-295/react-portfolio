...
const baseBadgeClasses =
  "bg-blue-100 text-blue-700 border-blue-200 font-bold dark:font-normal";

const badgeColors = {
  green: `${baseBadgeClasses} dark:bg-green-900/30 dark:text-green-400 dark:border-green-800`,
  blue: `${baseBadgeClasses} dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800`,
};
...
export default function TimelineItem({
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-between w-full mb-12 relative">
      ...
      >
        <div
          className={`inline-block px-4 py-2 rounded-lg bg-white dark:bg-card-bg border border-light-border dark:border-white/10 shadow-micro dark:shadow-none mb-2 ${isRight ? "md:mr-0" : "md:ml-0"}`}
        >
          <span className="text-secondary-dark dark:text-accent-teal font-mono font-bold text-sm tracking-wider">
            {period}
          </span>
        </div>
        <p className="text-light-text-secondary dark:text-slate-400 font-medium text-sm mt-1">
          {subtitle}
        </p>
      </div>
      ...
      <div
        className={`hidden md:block absolute top-1/2 w-12 h-0.5 bg-light-border dark:bg-white/10 -translate-y-1/2 z-0 ${isRight ? "left-1/2" : "right-1/2"}`}
      ></div>
      ...
      >
        <div
          className={`w-2 h-2 rounded-full ${
            dotBgColors[dotColor] || dotBgColors.blue
          }`}
        ></div>
      </div>
      ...
      >
        <div className="bg-gray-50 dark:bg-transparent border border-gray-100 dark:border-white/10 p-8 rounded-xl relative hover:border-gray-900 dark:hover:border-primary-blue/30 transition-transform hover:-translate-y-1 duration-300 shadow-sm dark:shadow-none hover:shadow-md dark:hover:shadow-none group">
          ...
          {institution && (
            <p className="text-gray-600 dark:text-slate-400 font-medium dark:font-normal text-sm mb-4">
              {institution}
            </p>
          )}
